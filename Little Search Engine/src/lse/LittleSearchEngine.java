	...
	public ArrayList<String> top5search(String kw1, String kw2) {
		...
		if(first == null || first.size() == 0) {
			for(int i = 0; i < second.size(); i++) {
				...
				if(!top5.contains(second.get(i).document)) {
					top5.add(second.get(i).document);
				}
			}
		}
		...
		while(first!=null&&second!=null&&indAr1 < first.size() && indAr2 < second.size()) {
		...
		if(first!=null&&indAr1 < first.size()) {
			isEmpty1 = true;
		}else if(second!=null&&indAr2 < second.size()) {
			isEmpty2 = true;
		}
